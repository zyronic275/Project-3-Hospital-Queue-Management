...
for index, p in enumerate(patients):
    ...
    if p["id"] > current_max_id:
        current_max_id = p["id"]

@app.get("/patients", response_model=List[schemas.Patient])
def get_patients(skip: int = 0, limit: int = 10):
    return patients[skip : skip + limit]

@app.get("/patients/{patient_id}", response_model=schemas.Patient)
def get_patient(patient_id: int):
    # Fixed: Search for the patient with this specific ID
...
def create_patient(patient: schemas.PatientCreate):
    ...
    current_max_id += 1
    new_patient_data = patient.dict()
    new_patient_data["id"] = current_max_id
    
    patients.append(new_patient_data)
    storage.write_data(patients)
    return new_patient_data

@app.put("/patients/{patient_id}", response_model=schemas.Patient)
def update_patient(patient_id: int, patient: schemas.PatientCreate):
    # Fixed: Find the index of the patient with this ID
    # We need the index to replace the item in the list
    target_index = next((i for i, p in enumerate(patients) if p["id"] == patient_id), None)
    
    if target_index is None:
        raise HTTPException(status_code=404, detail="Patient not found")
    
    # Keep the ID, update the rest
    updated_data = patient.dict()
    updated_data["id"] = patient_id
    
    patients[target_index] = updated_data
    storage.write_data(patients)
    return updated_data
